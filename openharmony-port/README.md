...
- 2026-05-10 起，OpenHarmony 版优先适配 1920x1080 教学屏：顶部工具栏紧凑化，指令编辑区与素材栏左右相邻，日志改为底部抽屉，右侧辅助栏和素材栏可调宽。
- OpenHarmony 展示中的“香橙派 / 软总线”锚点已放入画布缩放层，可随积木同步缩放和平移。
...
6. 检查页面是否加载：
   ...
   - 汇编预览
   - 执行日志
7. 检查顶部运行环境标识是否显示 `OH 1080p`，悬停可查看实际 ArkWeb 视口。
8. 验证不显示数据流动画面板。
9. 验证拖拽、吸附、单步执行、上一步、保存导出是否仍可用。
10. 打开 OpenHarmony 展示，验证软总线锚点随画布缩放同步变化。
