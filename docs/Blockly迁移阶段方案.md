...
> 2026-05-03 更新：本方案已经完成阶段性技术验证。根据当前产品决策，Blockly 迁移版不再作为最终产品主线，而是保留为技术验证和工程参考。正式主线回到 `app/` 的非 Blockly 自研积木界面，并优先改造成桌面版。详见 `docs/技术路线决策与阶段复盘.md`。
...
5. 数据流动画暂时不修改视觉方案，迁移时先复用当前 SVG/组件化思路。
